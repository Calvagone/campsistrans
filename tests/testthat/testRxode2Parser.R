...
test_that("Test the rxode2 parser (complex if statements, equations, line breaks, etc.)", {
  ...
  expected <- list() %>%
    ...
    append(
      ComplexIfElseStatement() %>%
        add(ExtendedIfStatement(
          "A==0 || (A==1 && A==3)",
          toModelStatements(Equation("OUTPUT", "1"))
        )) %>%
        add(ElseIfStatement(
          "A==1",
          toModelStatements(Equation("OUTPUT", "2"))
        )) %>%
  ...
  complexIfStatementStrB <- campsismod::to_string(
    complexIfStatement,
    dest = "NONMEM"
  ) # Test the dest attribute
  expect_error(campsismod::to_string(complexIfStatement, dest = "unknown"))

  expect_equal(
    complexIfStatementStrA,
    c(
      "if (A==0 || (A==1 && A==3)) {\n  OUTPUT=1\n}",
      "else if (A==1) {\n  OUTPUT=2\n}",
      "else {\n  OUTPUT=3\n}"
  ...
  complexIfStatement <- complexIfStatement %>%
    campsismod::replace_all(
      pattern = VariablePattern("OUTPUT"),
      replacement = "OUTPUT2"
    )
  complexIfStatementStrA <- campsismod::to_string(complexIfStatement) # Default destination
  expect_equal(
  ...
  expected <- list() %>%
    append(
      ComplexIfElseStatement() %>%
        add(ExtendedIfStatement(
          "NbCibleEH == 0",
          toModelStatements(Equation("tNbCibleEH", "\"G_0\""))
        )) %>%
        add(ElseIfStatement(
          "NbCibleEH == 1 || NbCibleEH == 2 || NbCibleEH == 3 || NbCibleEH == 4 || NbCibleEH == 5",
