...
etas <- (model@parameters %>% select("omega"))@list %>%
  purrr::map_chr(~ paste0("ETA_", .x@name))
nonmemDataset <- dataset %>%
  export(dest = "RxODE", model = model) %>%
  select(-all_of(etas))
nonmemDataset <- nonmemDataset %>%
  left_join(results %>% transmute(ID = id, TIME = time, EVID = 0, DV = Y)) %>%
  campsisqual::standardiseDataset()
