...
test_that("Unsupported error models are ignored (warning raised)", {
  ...
  model <- expect_warning(
    importRxode2(rxmod()),
    regexp = "Syntax error in error model code. Error model ignored."
  )
  error <- model %>%
    find(ErrorRecord())
...
test_that("thetaMat in rxode2 is automatically converted to a variance-covariance matrix in Campsis", {
  ...
  rxmod <- function() {
    ...
    thetaMat <- lotri({
      ...
      V_pop ~ c(
        Tlag_pop = 0.000338525,
        ka_pop = -0.000664941,
        V_pop = 0.00077728
      )
      beta_V_logtWT ~ c(
        Tlag_pop = -7.37595e-05,
        ka_pop = -0.00173821,
        V_pop = -0.000187249,
        beta_V_logtWT = 0.0207254
      )
      Cl_pop ~ c(
        Tlag_pop = 0.000359372,
        ka_pop = -0.000799374,
        V_pop = 1.20039e-05,
        beta_V_logtWT = 1.73146e-06,
        Cl_pop = 0.00208597
      )
      beta_Cl_logtAGE ~ c(
        Tlag_pop = -0.000427923,
        ka_pop = 0.00137366,
        V_pop = -4.68529e-05,
        beta_V_logtWT = -8.19417e-05,
        Cl_pop = 0.00056905,
        beta_Cl_logtAGE = 0.0232414
      )
      beta_Cl_logtWT ~ c(
        Tlag_pop = -0.000294548,
        ka_pop = 0.000904956,
        V_pop = -3.88107e-05,
        beta_V_logtWT = -8.14008e-06,
        Cl_pop = -0.000583146,
        beta_Cl_logtAGE = 0.0035915,
      ...
      omega_Tlag ~ c(
        ...
        ka_pop = 0.00195888,
        V_pop = -0.000140117,
        beta_V_logtWT = -0.000156667,
        Cl_pop = -0.000128981,
        beta_Cl_logtAGE = 0.00013627,
        beta_Cl_logtWT = 0.000328784,
        omega_Tlag = 0.0138484
      )
      omega_ka ~ c(
        Tlag_pop = 0.00356298,
        ka_pop = 0.00210609,
        V_pop = 0.000128544,
        beta_V_logtWT = 0.000277115,
        Cl_pop = 0.000878141,
        beta_Cl_logtAGE = -0.00306933,
        beta_Cl_logtWT = -0.00279933,
        omega_Tlag = -0.00128196,
        omega_ka = 0.057906
      )
      omega_V ~ c(
        Tlag_pop = -0.000101699,
        ka_pop = -0.000324991,
        V_pop = 4.75454e-06,
        beta_V_logtWT = 0.000133395,
        Cl_pop = -3.00809e-06,
        beta_Cl_logtAGE = -2.41747e-05,
        beta_Cl_logtWT = -1.92085e-05,
        omega_Tlag = 0.000107963,
        omega_ka = 0.000143227,
        omega_V = 0.000490203
      )
      omega_Cl ~ c(
        Tlag_pop = -3.65558e-05,
        ka_pop = -8.56155e-05,
        V_pop = 7.19071e-06,
        beta_V_logtWT = 5.34602e-06,
        Cl_pop = 1.4775e-05,
        beta_Cl_logtAGE = 7.56588e-05,
        beta_Cl_logtWT = -1.88607e-05,
        omega_Tlag = 4.12211e-05,
        omega_ka = 2.07224e-05,
        omega_V = 1.27613e-05,
        omega_Cl = 0.00109385
      )
      a ~ c(
        Tlag_pop = 0.000301054,
        ka_pop = 0.000409364,
        V_pop = -3.53596e-05,
        beta_V_logtWT = -0.000226686,
        Cl_pop = 2.4098e-05,
        beta_Cl_logtAGE = 0.000212168,
        beta_Cl_logtWT = 0.000308927,
        omega_Tlag = -6.1295e-05,
        omega_ka = -0.000635849,
        omega_V = 3.66105e-06,
        omega_Cl = -9.13216e-06,
        a = 0.00135834
      )
      b ~ c(
        Tlag_pop = -6.48436e-05,
        ka_pop = -8.32097e-05,
        V_pop = 5.99048e-06,
        beta_V_logtWT = 3.39069e-05,
        Cl_pop = -8.82669e-06,
        beta_Cl_logtAGE = -1.45752e-05,
        beta_Cl_logtWT = -2.97896e-05,
        omega_Tlag = -1.15585e-05,
        omega_ka = -0.000152184,
        omega_V = -1.2131e-05,
        omega_Cl = -2.96343e-06,
        a = -0.00013078,
        b = 4.91237e-05
      )
    })
    ...
    model({
      ...
      Cl <- exp(
        Cl_pop +
          beta_Cl_logtAGE * log(AGE / 30.4353) +
          beta_Cl_logtWT * log(WT / 68.1178) +
          omega_Cl
      )
  ...
  rxmod <- function() {
    ...
    model({
      ...
      d / dt(Liver) <- QHA *
        ...
        VAB +
        QSP * Spleen / KbSP / VSP +
        QPA * Pancreas / KbPA / VPA +
        QST * Stomach / KbST / VST +
        QGU * Gut / KbGU / VGU -
        CLint * fub * Liver / KbLI / VLI -
        QLI * Liver / KbLI / VLI
      ...
      d / dt(Arterial_Blood) <- QLU *
        (Lungs / KbLU / VLU - Arterial_Blood / VAB)
      d / dt(Venous_Blood) <- QHT *
        Heart /
        KbHT /
        VHT +
        QBR * Brain / KbBR / VBR +
        QMU * Muscles / KbMU / VMU +
        QAD * Adipose / KbAD / VAD +
        QSK * Skin / KbSK / VSK +
        QLI * Liver / KbLI / VLI +
        QBO * Bones / KbBO / VBO +
        QKI * Kidneys / KbKI / VKI +
        QRB * Rest_of_Body / KbRB / VRB -
        QLU * Venous_Blood / VVB
      d / dt(Rest_of_Body) <- QRB *
        (Arterial_Blood / VAB - Rest_of_Body / KbRB / VRB)
