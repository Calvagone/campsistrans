...
generateModel <- function(
  ...
  covFile1 <- file.path(
    ...
    "monolix_models",
    folder,
    "covarianceEstimatesSA.txt"
  )
  covFile2 <- file.path(
    testFolder,
    "monolix_models",
    folder,
    "covarianceEstimatesLin.txt"
  )
...
validateModelImplementation <- function(
  folder,
  model,
  dataset,
  output,
  ...
  predictions <- read.csv(file.path(
    testFolder,
    "monolix_models",
    folder,
    "predictions.txt"
...
test_that("PK_01 can be imported successfully", {
  ...
  nonreg_model <- suppressWarnings(read.campsis(nonRegressionFolderPath(
    folder
  )))
...
test_that("Warfarin PKPD IRM can be imported successfully", {
  ...
  modelFun <- function(model) {
    model <- model %>%
      campsismod::move(
        x = Equation("k"),
        to = campsismod::Position(OdeRecord(), after = FALSE)
      )
    return(model)
  }

  model <- expect_warning(
    generateModel(folder = folder, modelFun, digits = 6),
    regexp = "The following parameters are missing from the thetaMat covariance matrix because they were"
  )
  nonreg_model <- suppressWarnings(read.campsis(nonRegressionFolderPath(
    folder
  )))
