...
test_that("ADVAN3 TRANS4 - simulation", {
  ...
  rxodeMod <- object %>% export(dest = "campsis") %>% export(dest = "rxode2")
  ...
  sim <- rxode2::rxSolve(
    mod,
    params = rxodeMod@theta,
    ev,
    omega = rxodeMod@omega,
    sigma = rxodeMod@sigma,
    nSub = 100
  )
...
test_that("removePiecewiseStatements method works as expected", {
  ...
  model_ <- model %>%
    replace(Ode(
      "A_CENTRAL",
      "-CL*A_CENTRAL/V + Piecewise((RATE, t < AMT/RATE), (0, True))"
    ))
...
test_that("RATE is properly removed from control stream", {
  expect_equal(
    removeRateFromString(
      "$INPUT ID ARM TIME EVID MDV AMT CMT RATE DOSENO DV DOSE TDOS TSLD"
    ),
  ...
  expect_equal(
    removeRateFromString("$INPUT ID ARM TIME EVID MDV AMT CMT $DATA RATE"),
  ...
  expect_equal(
    removeRateFromString(filgrastim),
  ...
  expect_equal(
    removeRateFromString(
      "$INPUT ID ARM TIME EVID MDV AMT CMT RATE2 RATE DOSENO DV DOSE TDOS TSLD"
    ),
    "$INPUT ID ARM TIME EVID MDV AMT CMT RATE2 DOSENO DV DOSE TDOS TSLD"
  )
})
