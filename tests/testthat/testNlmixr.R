...
test_that("Conversion to nlxmir works as expected (Pharmpy)", {
  ...
  campsistrans <- importNONMEM(
    getNONMEMModelTemplate(4, 4),
  ...
  estimationStep <- pharmpy$modeling$estimation_steps$EstimationStep(
    method = "FOCE"
  )
  ...
  code <- code[
    !grepl("\\s+Y ~ add\\(add_error\\) \\+ prop\\(prop_error\\)", code)
  ]
