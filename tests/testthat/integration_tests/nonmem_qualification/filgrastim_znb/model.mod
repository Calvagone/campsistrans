...
$INPUT ID ORIGINAL_ID TIME DV MDV EVID AMT CMT RATE ROUT BAS ETA_NB0 ETA_KEL ETA_VD ETA_KA ETA_KSI ETA_SC1 ETA_SM1 ETA_SM2
...
$PK
   ...
   FF    = THETA(1)
KA = THETA(2)*EXP(ETA_KA)
   FR    = THETA(3)
   D2    = THETA(4)
KEL = THETA(5)*EXP(ETA_KEL)
VD = THETA(6)*EXP(ETA_VD)
   KD    = THETA(7)
   KINT  = THETA(8)
KSI = THETA(9)*EXP(ETA_KSI)
   KOFF  = THETA(10)
   KMT   = THETA(11)
   KBB1  = THETA(12)
   KTT   = THETA(13)    
NB0 = THETA(14)*EXP(ETA_NB0)
SC1 = THETA(15)*EXP(ETA_SC1)
SM1 = THETA(16)*EXP(ETA_SM1)
SM2 = THETA(17)*EXP(ETA_SM2)
   SM3    = THETA(18)
   CP0   = BAS
    F1=FF
    F2=0
...
$TABLE ID TIME EVID MDV DV AMT CMT ZNB FILE=output.tab ONEHEADER NOAPPEND NOPRINT
