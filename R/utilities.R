    ...
    print <- tryCatch(
      {
        gsub(
          pattern = "^/\\*.*\\*/\\s*",
          replacement = "",
          x = reticulate::py_capture_output(sympy$print_ccode(
            expr,
            strict = FALSE
          ))
        )
...
removeRateFromString <- function(x) {
  ...
  retValue <- gsub(
    pattern = "^(.*)(\\$INPUT)([^\\$]*)([[:space:]]+RATE[[:space:]]+)(.*)",
    replacement = "\\1\\2\\3 \n\\5",
    x = retValue
  )
...
nameCovariance <- function(model) {
  ...
  for (listIndex in seq_len(length(parameters))) {
    ...
    if (is(parameter, "omega") && !parameter %>% campsismod::is_diag()) {
      ...
      parameter <- standardiseCovarianceParameterName(
