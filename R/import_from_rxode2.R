...
#' @export
importRxode2 <- function(
  rxmod,
  pop_parameter_regex = NULL,
  omega_parameter_regex = NULL,
  subroutine = NULL,
  cov = TRUE
) {
  ...
  # Extract parameters
  model@parameters <- extractParametersFromRxode(
    rxmod,
  ...
  for (parameter in model@parameters@list %>% purrr::keep(~ is(.x, "theta"))) {
    ...
    if (
      !is.na(parameter@comment) &&
        parameter@comment == "Population parameter" &&
        !is.null(pop_parameter_regex)
    ) {
      oldNameInCode <- restorePrefixSuffix(
        x = oldNameInCode,
        regex = pop_parameter_regex
      )
    }
    ...
    # Adapt varcov
    varcov <- adaptVarcovName(
      varcov = varcov,
      before = oldNameInCode,
      after = updatedThetaName
    )
  }
...
extractModelCodeFromRxode <- function(rxmod, subroutine) {
  ...
  if (!is.null(subroutine)) {
    ...
    assertthat::assert_that(
      length(subroutine) == 2L,
      msg = "Subroutine should be a vector of 2 integers"
    )
  ...
  for (cmtIndex in cmtIndexes) {
    code <- renameCompartmentProperties(
      code = code,
      occurrence = sprintf("rxddta%s", cmtIndex),
      replacement = as.character(cmtIndex),
      prefix = "A"
    )
  }
  ...
  if (!is.null(subroutineModel)) {
    for (cmtName in c("depot", "central")) {
      ...
      code <- replace_all(
        object = code,
        pattern = Pattern(sprintf("\\(%s\\)", cmtName)),
  ...
  for (cmtName in cmtNames) {
    code <- replace_all(
      object = code,
      pattern = VariablePattern(cmtName),
...
convertComplexIfElseStatement <- function(x) {
  ...
  assertthat::assert_that(
    !is.null(ifStatement),
    msg = "Complex if statement without if clause"
  )
  ...
  statements@list <- statements@list %>%
    purrr::map(.f = function(statement) {
      if (is(statement, "equation")) {
        ...
        return(Equation(
          lhs = sprintf("FIXME%i", fixmeIndex),
          rhs = "0",
...
extractCompartmentPropertiesFromRxode <- function(model) {
  ...
  indexes <- ode@statements@list %>%
    purrr::map_lgl(
      ~ is(.x, "unknown_statement") &&
        isRxodeCompartmentPropertyEquation(.x@line)
    ) %>%
    which()
        ...
        compartmentIndex <- tryCatch(
          {
            get_compartment_index(
              ...
              name = gsub(
                pattern = "A_",
                replacement = "",
                x = compartmentNameWithA
              )
            )
          },
          error = function(e) {
            warning(sprintf(
              "Compartment %s not found in model, compartment property linked to first compartment.",
              compartmentNameWithA
            ))
            return(1L)
          }
        )
...
extractInitialConditionsFromRxode <- function(model) {
  ...
  indexes <- ode@statements@list %>%
    purrr::map_lgl(
      ~ is(.x, "unknown_statement") && isRxodeInitialConditionEquation(.x@line)
    ) %>%
    which()
  ...
  initialConditions <- ode@statements@list[indexes] %>%
    purrr::map(
      ~ {
        ...
        rhs <- extract_rhs(.x@line) %>% trimws()
        compartmentIndex <- get_compartment_index(
          object = model,
          name = gsub(
            pattern = "^A_",
            replacement = "",
  ...
  model@compartments@properties@list <- c(
    model@compartments@properties@list,
    initialConditions
  )
...
#' @return a functional Campsis model
#' @export
extractParametersFromRxode <- function(
  rxmod,
  pop_parameter_regex,
  omega_parameter_regex
) {
  ...
  parameters@list <- parameters@list %>%
    purrr::map(.f = function(.x) {
      popSuffixDetected <- detectPrefixSuffix(
        x = .x@name,
        regex = pop_parameter_regex
      )
  ...
  assertthat::assert_that(all(
    dimnames(omegaMatrix)[[1]] == dimnames(omegaMatrix)[[2]]
  ))
  omegaNames <- dimnames(omegaMatrix)[[1]]

  # Take into account omega parameter regex
  omegaNames <- removePrefixSuffix(
    x = omegaNames,
    regex = omega_parameter_regex
  )
    ...
    for (index2 in seq_along(omegaNames)) {
      ...
      if (omegaValue != 0 || index1 == index2) {
        ...
        omega <- Omega(
          ...
          index = index1,
          index2 = index2,
          type = ifelse(index1 == index2, "var", "covar")
        )
        parameters <- parameters %>%
          add(omega)
      }
...
heuristicMoveToMain <- function(model) {
  ...
  for (continuousVariable in continuousVariables) {
    oldOdeTmp <- oldOdeTmp %>%
      replace_all(
        pattern = VariablePattern(continuousVariable),
        replacement = ""
      )
  }
...
convertRxodeErrorModel <- function(model, rxmod) {
  ...
  errorEquationLgl <- ode@statements@list %>%
    purrr::map_lgl(.f = function(statement) {
      if (
        is(statement, "unknown_statement") &&
          isRxodeErrorEquation(statement@line)
      ) {
        return(TRUE)
      } else {
        return(FALSE)
...
isRxodeCompartmentPropertyEquation <- function(x) {
  ...
  return(grepl(
    pattern = paste0(
      "^(f|alag|dur|rate)\\(",
      campsismod:::variable_pattern_str(),
      "\\)$"
    ),
    x = parts[1] %>% trim()
  ))
...
isRxodeInitialConditionEquation <- function(x) {
  ...
  return(grepl(
    pattern = sprintf("^%s\\(0\\)$", campsismod:::variable_pattern_str()),
    x = parts[1] %>% trim()
  ))
}
...
getSubroutineModelForRxode2 <- function(advan, trans) {
  model <- campsismod::model_suite$nonmem[[paste0(
    "advan",
    advan,
    "_trans",
    trans
...
replaceLinCmt <- function(model, subroutineModel) {
  ...
  equationIndex <- ode@statements@list %>%
    purrr::detect_index(
      .f = detectFun <- function(statement) {
        if (is(statement, "equation")) {
          return(grepl(
            pattern = "(linCmt\\(\\))|(linCmtFun)",
            x = statement@rhs
          ))
        } else {
          return(FALSE)
...
renameCompartmentProperties <- function(code, occurrence, replacement, prefix) {
  ...
  code <- replace_all(
    object = code,
    pattern = VariablePattern(sprintf("rxf\\.%s\\.", occurrence)),
    replacement = sprintf("F_%s%s", prefix, replacement)
  )
  # Initial conditions
  code <- replace_all(
    object = code,
    pattern = VariablePattern(sprintf("rxini\\.%s\\.", occurrence)),
    replacement = sprintf("INIT_%s%s", prefix, replacement)
  )
  # Infusion durations
  code <- replace_all(
    object = code,
    pattern = VariablePattern(sprintf("rxdur\\.%s\\.", occurrence)),
    replacement = sprintf("DUR_%s%s", prefix, replacement)
  )
  # Infusion rates
  code <- replace_all(
    object = code,
    pattern = VariablePattern(sprintf("rxrate\\.%s\\.", occurrence)),
    replacement = sprintf("RATE_%s%s", prefix, replacement)
  )
  # Lag times
  code <- replace_all(
    object = code,
    pattern = VariablePattern(sprintf("rxalag\\.%s\\.", occurrence)),
...
adaptVarcovName <- function(varcov, before, after) {
  ...
  row.names(varcov) <- gsub(
    pattern = regex,
    replacement = after,
    x = row.names(varcov)
  )
  colnames(varcov) <- gsub(
    pattern = regex,
    replacement = after,
    x = colnames(varcov)
  )
