...
importNONMEM <- function(
  ...
  if (copy_dir) {
    ...
    specificDir <- file.path(
      tmpDir,
      paste0("dir_", sample(x = 1e9, 1, replace = TRUE))
    )
    if (!dir.exists(specificDir)) {
      dir.create(specificDir)
    }
  ...
  adaptNONMEMControlStream(
    file = ctlPath,
    rem_rate = rem_rate,
    rem_abbr_replace = rem_abbr_replace
  )
  ...
  if (
    estimate &&
      (mapping$params %>% campsismod::select("omega") %>% length() == 0)
  ) {
    omegaIndexes <- seq_len(getNumberOfEtas(model))
    mapping$params <- addMapping(
      omegaIndexes,
  ...
  if (
    mapping$auto &&
      !isTRUE(validObject(campsis@parameters, test = TRUE, complete = TRUE))
  ) {
    campsis <- tryCatch(
...
setMethod(
  "export",
  signature = c("campsistrans", "character"),
