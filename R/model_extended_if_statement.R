...
setMethod(
  "get_name",
  signature = c("extended_if_statement"),
...
setMethod(
  "replace_all",
  signature = c("extended_if_statement", "pattern", "character"),
...
setMethod(
  ...
  definition = function(object, ...) {
    dest <- campsismod::process_extra_arg(
      args = list(...),
      name = "dest",
      default = "campsis"
    )
    ...
    statementsStr <- object@statements@list %>%
      purrr::map_chr(.f = function(statement) {
        return(paste0(
          indent,
          statement %>% campsismod::to_string(dest = dest, init = FALSE)
        ))
      }) %>%
      paste0(collapse = "\n")
