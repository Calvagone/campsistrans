...
addMapping <- function(x, parameters, type) {
  if (!is.null(x)) {
    assertthat::assert_that(
      is.numeric(x),
      msg = paste0(type, " is not numeric")
    )
    names <- names(x)
    if (is.null(names)) {
...
convertParameters <- function(model, mapping) {
  assertthat::assert_that(
    inherits(model, "pharmpy.model.model.Model"),
    msg = "model is not a Pharmpy model"
  )
  if (!is.null(mapping)) {
    assertthat::assert_that(
      inherits(mapping, "pmxmapping"),
...
retrieveInitialValues <- function(parset) {
  assertthat::assert_that(
    inherits(parset, "pharmpy.model.parameters.Parameters"),
    msg = "parset is not a parameter set"
  )

  paramsList <- purrr::map2(
    parset$inits,
    names(parset$inits),
    .f = function(initialValue, name) {
      fix <- as.logical(parset$fix[name])
      return(convertPharmpyParameter(
        name = name,
        value = initialValue,
        fix = fix
      ))
...
convertPharmpyParameter <- function(name, value, fix) {
  ...
  if (is.null(type)) {
    stop(paste0(
      "Unknown parameter ",
      name,
      ": estimated parameter type must be THETA, OMEGA or SIGMA."
    ))
  }
