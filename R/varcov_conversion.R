...
convertVarcov <- function(varcov, parameters) {
  ...
  nmNames <- parameters@list %>%
    purrr::map_chr(.f = ~ .x %>% campsismod::get_nonmem_name())

  # Retrieve varcov parameters
  varcovParams <- colnames(varcov) %>%
    ...
    if (length(parameter) == 0) {
      if (!is(varcovParam, "theta") && !varcovParam %>% campsismod::is_diag()) {
        ...
        stop(paste0(
          "Parameter ",
          nmName,
          " can't be found in model parameters."
        ))
      }
    } else {
