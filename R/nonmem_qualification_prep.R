...
standardiseDataset <- function(dataset) {
  ...
  dataset <- dataset %>%
    dplyr::mutate_at(.vars = colnames(dataset), .funs = as.character)

  # Replace NAs by dots
  dataset <- dataset %>%
    dplyr::mutate_at(
      .vars = colnames(dataset),
      .funs = ~ ifelse(is.na(.x), ".", .x)
    )
...
executeSimulationCtl <- function(
  ...
  for (blockName in c("PK", "ERROR")) {
    currentBlock <- extractNONMEMBlock(
      x = ctl,
      name = blockName,
      first = TRUE,
      raise_error = FALSE
    ...
    if (length(currentBlock) > 0) {
      ctl <- replaceNONMEMBlock(
        x = ctl,
        name = blockName,
        content = setEtasAsCovariates(model, currentBlock@content)
      )
    }
  }
  ...
  tableStr <- sprintf(
    "$TABLE %s FILE=output.tab ONEHEADER NOAPPEND NOPRINT\n",
  ...
  ctl <- replaceNONMEMBlock(
    x = ctl,
    name = "INPUT",
    content = colnames(dataset) %>% paste0(collapse = " ")
  )
  ...
  write.csv(
    dataset,
    file = file.path(folder, "dataset.csv"),
    quote = FALSE,
    row.names = FALSE
  )
...
executeNONMEM <- function(folder, reexecuteNONMEM = T, ctl_name = "model.mod") {
  ...
  if (!file.exists(tabFile) || reexecuteNONMEM) {
    system(
      "cmd.exe",
      input = paste0("cd ", "\"", folder, "\"", " & ", "execute ", ctl_name)
    )
...
updateInitsNONMEM <- function(ctl) {
  ...
  system(
    "cmd.exe",
    input = sprintf(
      "cd \"%s\" & update_inits %s %s -output_model=\"%s\"",
      folder,
      basename(ctl),
      basename(lst),
      output
...
setEtasAsCovariates <- function(model, content) {
  ...
  for (omega in omegas@list) {
    ...
    index <- omega@index
    content <- gsub(
      pattern = sprintf("(?<![A-Z0-9_])ETA\\(%i\\)", index),
      replacement = etaName,
      x = content,
      perl = TRUE
    )
