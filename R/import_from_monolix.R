copyAndRename <- function(file, tempDir, newName) {
  ...
  file.rename(
    from = file.path(tempDir, basename(file)),
    to = file.path(tempDir, newName)
  )
...
importMonolix <- function(
  ...
  mlxtran <- copyAndRename(
    file = mlxtranFile,
    tempDir = tempDir,
    newName = "project.mlxtran"
  )
  ...
  if (length(longitudinalIndex) > 0) {
    ...
    if (length(filePathIndexes) > 0) {
      ...
      mlxtranStr <- mlxtranStr %>%
        append(sprintf("file = '%s'", modelBasename), after = longitudinalIndex)
    }
  }
  ...
  if (estimate) {
    copyAndRename(
      file = parametersFile,
      tempDir = exportDir,
      newName = "populationParameters.txt"
    )
  }
  ...
  if (!is.null(covFile) && file.exists(covFile)) {
    ...
    copyAndRename(
      file = covFile,
      tempDir = fisherInformationDir,
      newName = "covarianceEstimatesSA.txt"
    )
  }
      ...
      if (length(parts) > 1) {
        ...
        if (model %>% campsismod::contains(Equation(variable))) {
          model <- model %>%
            campsismod::move(
              x = Equation(variable),
              to = campsismod::Position(MainRecord())
            )
        }
      }
    }
