...
Rxode2Lexer <- R6::R6Class(
  ...
  public = list(
    ...
    t_EQUATION = function(re = '[a-zA-Z_][a-zA-Z0-9_\\.]*\\s*=\\s*[^\n]+', t) {
      ...
      if (self$debug) {
        message("Found equation: ", t$value)
      }
      return(t)
    },
    ...
    t_LINE_BREAK = function(re = '\n[ ]*\n', t) {
      if (self$debug) {
        message("Found line break: ", t$value)
      }
      return(t)
    },
    ...
    t_CONDITION = function(re = '\\((?>[^()]|(?R))*\\)', t) {
      if (self$debug) {
        message("Found condition: ", t$value)
      }
      return(t)
    },
    ...
    t_IF_CONTENT = function(re = '\\{(?>[^{}]|(?R))*\\}', t) {
      if (self$debug) {
        message("Found if content: ", t$value)
      }
      return(t)
    },
    ...
    t_UNKNOWN_STATEMENT = function(re = '.*[^\n]+', t) {
      if (self$debug) {
        message("Found unknown statement: ", t$value)
      }
      return(t)
    },
...
Rxode2Parser <- R6::R6Class(
  ...
  public = list(
    ...
    p_model_statements = function(
      doc = 'model_statements : model_statement
                         | model_statements model_statement',
      p
    ) {
      if (p$length() == 2) {
        p$set(1, p$get(2))
    ...
    #' @param doc doc argument
    #' @param p parser object
    p_model_statement = function(
      doc = 'model_statement : complex_if_else_statement
                         | equation 
                         | ode
                         | line_break
                         | comment
    ...
    #' @param doc doc argument
    #' @param p parser object
    p_complex_if_else_statement = function(
      doc = 'complex_if_else_statement : if_statement
                         | complex_if_else_statement if_statement',
      p
    ) {
      if (p$length() == 2) {
        ifStatement <- p$get(2)
        ...
        if (
          is(ifStatement, "else_if_statement") ||
            is(ifStatement, "else_statement")
        ) {
          complexIfElseStatement1[[lastElem]]@list <- append(
            complexIfElseStatement1[[lastElem]]@list,
            list(ifStatement)
          )
    ...
    #' @param doc doc argument
    #' @param p parser object
    p_simple_if_statement = function(
      doc = 'simple_if_statement : IF CONDITION EQUATION',
      p
    ) {
      p$set(
        1,
    ...
    #' @param doc doc argument
    #' @param p parser object
    p_if_statement = function(
      doc = 'if_statement : IF CONDITION EQUATION
      ...
      if (p$get(2) == 'if') {
        ...
        p$set(
          1,
          buildIfStatement(condition = "()", content = p$get(3), type = "else")
        )
      }
    },
    ...
    p_ode = function(doc = 'ode : ODE', p) {
      ...
      rhs <- extract_rhs(p$get(2)) %>% trimws()
      ode <- Ode(
        lhs = sprintf("%s", extract_text_between_brackets(lhs)),
        rhs = getExpr(rhs),
        comment = getComment(rhs)
      )
      p$set(1, ode)
    ...
    #' @param doc doc argument
    #' @param p parser object
    p_unknown_statement = function(
      doc = 'unknown_statement : UNKNOWN_STATEMENT',
      p
    ) {
      p$set(
        1,
        UnknownStatement(
          line = getExpr(p$get(2)),
          comment = getComment(p$get(2))
        )
      )
    },
    ...
    p_error = function(p) {
      ...
      if (is.null(p)) {
        ...
        print(sprintf("Syntax error at '%s'", p$value))
      }
    }
  )
)
