...
autoExtractParameters2 <- function(model) {
  ...
  for (omega in unusedOmegas) {
    model <- deleteParameterAndCorrelations(
      model = model,
      parameter = Omega(name = omega)
    )
  }
  for (sigma in unusedSigmas) {
    model <- deleteParameterAndCorrelations(
      model = model,
      parameter = Sigma(name = sigma)
    )
  }
...
deleteParameterAndCorrelations <- function(model, parameter) {
  ...
  model@parameters@list <- model@parameters@list %>%
    purrr::discard(
      .p = ~ (as.character(class(.x)) == type) &&
        ((.x@index %in% indexes) || (.x@index2 %in% indexes))
    )
  return(model)
}

extractParametersCore <- function(record) {
  retValue <- record@statements@list %>%
    purrr::map(.f = function(statement) {
      ...
      return(list(
        thetas = extractParameters(text = text, type = "THETA"),
...
extractParameters <- function(text, type) {
  text_ <- paste0(" ", text)
  retValue <- gregexpr(
    pattern = sprintf("[^a-zA-Z_](%s_)[A-Za-z0-9_]+", type),
    text = text_
  )[[1]]
  ...
  return(purrr::map2_chr(
    .x = start,
    .y = length,
    .f = ~ substring(text_, first = .x + nchar(type) + 2, last = .x + .y - 1)
  ))
}
...
repairParametersByType <- function(parameters, type) {
  ...
  onDiagParameters@list <- onDiagParameters@list %>%
    purrr::keep(
      ~ !(is(.x, "double_array_parameter") && (.x@index != .x@index2))
    )
  ...
  offDiagParameters_ <- offDiagParameters@list %>%
    purrr::map(
      ~ list(
        parameter = .x,
        name1 = onDiagParameters %>%
          campsismod::get_by_index(Omega(
            index = .x@index,
            index2 = .x@index
          )) %>%
          .@name,
        name2 = onDiagParameters %>%
          campsismod::get_by_index(Omega(
            index = .x@index2,
            index2 = .x@index2
          )) %>%
  ...
  for (offDiagParameter_ in offDiagParameters_) {
    ...
    offDiagParameter@index <- retValue %>%
      campsismod::find(Omega(name = name1)) %>%
      .@index
    offDiagParameter@index2 <- retValue %>%
      campsismod::find(Omega(name = name2)) %>%
      .@index
    retValue <- retValue %>%
