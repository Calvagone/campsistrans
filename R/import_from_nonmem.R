...
importNONMEM2 <- function(ctlFile, extFile = NULL, covFile = NULL) {
  ...
  model@parameters@list <- model@parameters@list %>%
    purrr::map(
      ~ standardiseCovarianceParameterName(
  ...
  for (index1 in seq_along(diag)) {
    for (index2 in seq_along(diag)) {
      ...
      if (onDiag) {
        ...
        sigma <- Sigma(
          name = replaceEpsInSigma(epsName),
          index = index1,
          index2 = index2,
          value = omegaValue,
          fix = omegaValue == 1,
...
detectSubroutine <- function(x) {
  indexes <- grepl(
    pattern = "^\\$(SUB|SUBR|SUBROUTINE|SUBROUTINES)\\s+.*",
    x = trimws(x)
  )
  ...
  advan <- suppressWarnings(as.numeric(gsub(
    pattern = ".*ADVAN([0-9]+).*",
    "\\1",
    subroutine
  )))
  trans <- suppressWarnings(as.numeric(gsub(
    pattern = ".*TRANS([0-9]+).*",
    "\\1",
    subroutine
  )))
...
replaceDatasetTime <- function(file) {
  ...
  lines <- replace_all(
    object = lines,
    pattern = VariablePattern("TIME"),
    replacement = "DATASET_TIME"
  )
