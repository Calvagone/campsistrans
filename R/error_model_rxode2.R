...
errorModelToCampsis <- function(x, shift) {
  assertthat::assert_that(
    length(x@add) <= 1,
    msg = "Slot 'add' should be length <= 1"
  )
  assertthat::assert_that(
    length(x@prop) <= 1,
    msg = "Slot 'prop' should be length <= 1"
  )
  ...
  if (x@combined1 || x@combined2) {
    ...
    if (length(x@add) == 1 && length(x@prop) == 0) {
      eq <- Equation(
        lhs = endpointErr,
        rhs = sprintf("%s + %s*EPS_FIX%i", endpoint, x@add, 1L + shift)
      )
      return(list(equation = eq, eps = c(1) + shift))
